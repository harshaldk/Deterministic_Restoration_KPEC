...
The objects for circuit, bus, and branch are used to derive the circuit initial state
This file also includes function to modify the base DSS circuit with sectionalizing, tie switch, and generator information.
The translation of DSS Circuit into a graph structure is defined here.
"""
import os
import copy
...
def CktModSetup(DSSfile): # give tie switches and sectionalizing switches as input
...
def initialize():       
    ...
    DSSCktobj = CktModSetup(DSSfile) # initially the sectionalizing switches close and tie switches open
    ...
    return DSSCktobj,G_init,conv_flag # Returns the DSSCircuit(base without outages), the corresponding graph, the convergence of power flow        